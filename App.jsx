...
function LicenseGate({ children }) {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        const { code, gymId } = JSON.parse(saved);
        setCode(code); setGymId(gymId); setStatus("unlocked");
  ...
  const activate = async () => {
    ...
    if (!trimmed) return setError("Enter your license code.");
    setLoading(true); setError("");
    try {
      // 1. Look up the code
      const rows = await sb.select("license_codes", `code=eq.${trimmed}&select=code,used`);
      if (!rows.length)  { setError("Code not found. Check for typos."); setLoading(false); return; }
      if (rows[0].used)  { setError("This code has already been activated on another device."); setLoading(false); return; }

      // 2. Ask for gym name
      const gymName = prompt("Enter your gym name:") || "My Gym";

      // 3. Create gym row
      const [gym] = await sb.insert("gyms", { license_code: trimmed, gym_name: gymName, device_id: deviceId, settings: {} });

      // 4. Mark code as used
      await sb.update("license_codes", { used: true, activated_at: new Date().toISOString(), gym_name: gymName, device_id: deviceId }, `code=eq.${trimmed}`);

      // 5. Save locally
      localStorage.setItem("gymos_license", JSON.stringify({ code: trimmed, gymId: gym.id }));
      setCode(trimmed); setGymId(gym.id); setStatus("unlocked");
    } catch(e) {
  ...
  if (status === "checking") return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"100vh", background:"#0a0a0f", fontFamily:"DM Sans, sans-serif" }}>
      <div style={{ color:"#e8ff47", fontFamily:"Bebas Neue, sans-serif", fontSize:32, letterSpacing:"0.1em" }}>LOADING…</div>
    </div>
  );

  if (status === "unlocked") return children({ gymId, licenseCode: code });
    ...
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"100vh", background:"#0a0a0f", fontFamily:"DM Sans, sans-serif", padding:20 }}>
      <div style={{ width:"100%", maxWidth:420, animation:"fadeUp 0.5s ease" }}>
        ...
        <div style={{ textAlign:"center", marginBottom:40 }}>
          <div style={{ display:"inline-flex", width:72, height:72, borderRadius:18, background:"#e8ff47", alignItems:"center", justifyContent:"center", fontFamily:"Bebas Neue, sans-serif", fontSize:38, color:"#000", marginBottom:16 }}>G</div>
          <div style={{ fontFamily:"Bebas Neue, sans-serif", fontSize:48, letterSpacing:"0.08em", color:"#f0f0f5", lineHeight:1 }}>GYM OS</div>
          <div style={{ color:"#8888a8", fontSize:14, marginTop:6 }}>Professional Gym Management</div>
        </div>
        ...
        <div style={{ background:"#14141e", border:"1px solid #2a2a3a", borderRadius:20, padding:32 }}>
          ...
          <div style={{ color:"#8888a8", fontSize:13, marginBottom:24, lineHeight:1.6 }}>Enter the license code you received after purchase. Each code activates one gym permanently.</div>

          <label style={{ display:"block", fontSize:11, color:"#555570", fontWeight:600, textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:8 }}>License Code</label>
          <input
            value={inputCode}
            onChange={e => { setInput(e.target.value.toUpperCase()); setError(""); }}
            onKeyDown={e => e.key === "Enter" && activate()}
            placeholder="GYMOS-XXXXX-XXXXX-XXXXX"
            style={{ width:"100%", background:"#1a1a24", border:`1px solid ${error ? "#f43f5e" : "#2a2a3a"}`, color:"#f0f0f5", fontFamily:"JetBrains Mono, monospace", padding:"12px 16px", borderRadius:10, fontSize:15, outline:"none", letterSpacing:"0.05em", marginBottom:error ? 8 : 16, boxSizing:"border-box" }}
          />
          {error && <div style={{ color:"#f43f5e", fontSize:12, marginBottom:16 }}>⚠ {error}</div>}

          <button
            onClick={activate}
            disabled={loading}
            style={{ width:"100%", padding:"13px 0", background:"#e8ff47", color:"#000", border:"none", borderRadius:10, fontFamily:"DM Sans, sans-serif", fontWeight:700, fontSize:15, cursor:loading?"not-allowed":"pointer", opacity:loading?0.7:1, transition:"opacity 0.2s" }}
          >{loading ? "Activating…" : "Activate & Launch →"}</button>

          <div style={{ marginTop:20, padding:"12px 16px", background:"#111118", borderRadius:8, fontSize:12, color:"#555570", lineHeight:1.7 }}>
            <div style={{ color:"#8888a8", fontWeight:600, marginBottom:4 }}>Don't have a code?</div>
            Contact your distributor or visit our website to purchase a license.
          </div>
        </div>

        <div style={{ textAlign:"center", marginTop:20, fontSize:11, color:"#333348" }}>
          GYM OS v2.0 · Licensed software · One-time activation
        </div>
      </div>
    </div>
  );
...
// ══════════════════════════════════════════════════════
//  DATA HOOKS  — sync all state with Supabase
// ══════════════════════════════════════════════════════
function useGymData(gymId) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setMembersRaw(mems.map(dbToMember));
        setTransactionsRaw(txs.map(dbToTx));
        setInventoryRaw(inv.map(dbToItem));
        setCheckinsRaw(chks.map(dbToCheckin));
  ...
  const setMembers = async (updater) => {
    setMembersRaw(prev => {
      ...
      added.forEach(m => sb.insert("members", { gym_id: gymId, name: m.name, email: m.email, phone: m.phone, offer_id: m.offerId, start_date: m.startDate, photo: m.photo, avatar: m.avatar }).catch(console.error));
      removed.forEach(m => sb.delete("members", `id=eq.${m.id}`).catch(console.error));
      changed.forEach(m => sb.update("members", { name: m.name, email: m.email, phone: m.phone, offer_id: m.offerId, start_date: m.startDate, photo: m.photo }, `id=eq.${m.id}`).catch(console.error));
      return next;
    });
  };
  ...
  const setTransactions = async (updater) => {
    setTransactionsRaw(prev => {
      ...
      const removed = prev.filter(t => !new Set(next.map(x=>x.id)).has(t.id));
      added.forEach(t => sb.insert("transactions", { gym_id: gymId, date: t.date, type: t.type, category: t.category, description: t.desc, amount: t.amount }).catch(console.error));
      removed.forEach(t => sb.delete("transactions", `id=eq.${t.id}`).catch(console.error));
      return next;
    });
  };

  const setInventory = async (updater) => {
    setInventoryRaw(prev => {
      ...
      const changed = next.filter(i => { const old=prev.find(p=>p.id===i.id); return old && JSON.stringify(old)!==JSON.stringify(i); });
      added.forEach(i => sb.insert("inventory", { gym_id: gymId, name: i.name, category: i.category, price: i.price, cost: i.cost, qty: i.qty }).catch(console.error));
      removed.forEach(i => sb.delete("inventory", `id=eq.${i.id}`).catch(console.error));
      changed.forEach(i => sb.update("inventory", { name: i.name, category: i.category, price: i.price, cost: i.cost, qty: i.qty }, `id=eq.${i.id}`).catch(console.error));
      return next;
    });
  };

  const setCheckins = async (updater) => {
    setCheckinsRaw(prev => {
      const next = typeof updater === "function" ? updater(prev) : updater;
      const added = next.filter(c => !prev.some(p => p.memberId===c.memberId && p.time===c.time));
      added.forEach(c => {
        // Find member uuid — c.memberId is already the uuid from DB
        sb.insert("checkins", { gym_id: gymId, member_id: c.memberId, checked_in_at: c.time }).catch(console.error);
      });
      return next;
    });
  };
